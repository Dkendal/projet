...
export function process(content: string, filename: string, config: any) {
  ...
  const loader = options?.loaders && options?.loaders[ext] ? options.loaders[ext] : extname(filename).slice(1) as Loader

  const sourcemaps = options?.sourcemap ? { sourcemap: true, sourcefile: filename } : {}
